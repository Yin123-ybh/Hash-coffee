  ...
  <div class="dashboard-container">
    ...
    <div class="container" :class="{ hContainer: tableData.length }">
      ...
      <el-table v-if="tableData.length" :data="tableData" stripe class="tableBox">
        ...
        <el-table-column
          ...
          :min-width="[2, 3, 4].includes(orderStatus) ? 130 : (orderStatus === 0 ? 140 : 100)"
        >
          <template slot-scope="{ row }">
            <div class="before">
              <el-button
    ...
    >
      <el-scrollbar style="height: 100%">
        <div class="order-top">
          <div>
            ...
            >
              {{ (orderList.find((item) => item.value === dialogOrderStatus) || { label: '' }).label }}
            </div>
          </div>
        </div>
          ...
          <div class="dish-info">
            ...
            <div class="dish-list">
              <div v-for="(item, index) in diaForm.orderDetailList" :key="index" class="dish-item">
                ...
                <span class="dish-price">￥{{ typeof item.amount === 'number' ? item.amount.toFixed(2) : '' }}</span>
              </div>
            </div>
            <div class="dish-all-amount">
              <label>咖啡小计</label>
              <span>￥{{ formatCurrency(dishesSubtotal) }}</span>
            </div>
          </div>
        </div>
          ...
          <div class="amount-info">
            ...
            <div class="amount-list">
              <div class="dish-amount">
                ...
                <span class="amount-price">￥{{ formatCurrency(dishesSubtotal) }}</span>
              </div>
              ...
              <div class="package-amount">
                ...
                <span class="amount-price">￥{{ formatCurrency(diaForm.packAmount) }}</span>
              </div>
              <div class="all-amount">
                <span class="amount-name">合计：</span>
                <span class="amount-price">￥{{ formatCurrency(diaForm.amount) }}</span>
              </div>
              <div class="pay-type">
      ...
      <span v-if="dialogOrderStatus !== 6" slot="footer" class="dialog-footer">
        ...
        <el-button v-if="dialogOrderStatus === 2" type="primary" @click="orderAccept(row); isTableOperateBtn = false">接 单</el-button>

        <el-button v-if="[1, 3, 4, 5].includes(dialogOrderStatus)" @click="dialogVisible = false">返 回</el-button>
        <el-button v-if="dialogOrderStatus === 3" type="primary" @click="cancelOrDeliveryOrComplete(3, row.id)">派 送</el-button>
        <el-button v-if="dialogOrderStatus === 4" type="primary" @click="cancelOrDeliveryOrComplete(4, row.id)">完 成</el-button>
...
@Component({
  components: { TabChange, Empty }
})
export default class extends Vue {
  // 常量
  private readonly DELIVERY_FEE: number = 6

  // 列表与查询
  private defaultActivity = 0
  private orderStatics: Record<string, any> = {}
  private input = ''
  private phone = ''
  ...
  private row: OrderRow = { id: '', amount: 0, status: 0 }
  ...
  // 详情表单（修复：由数组改为对象，并给出安全默认值）
  private diaForm: OrderDetail = {
    id: '',
    number: '',
    orderTime: '',
    consignee: '',
  ...
  created() {
    ...
    this.init(Number.isFinite(status) ? status : 0)
  }

  mounted() {
    // 如果有值说明是消息通知点击进来的
    if (this.$route.query.orderId && this.$route.query.orderId !== 'undefined') {
      this.goDetail(this.$route.query.orderId as any, 2)
    }
  ...
  // 价格格式化与小计（避免 NaN/undefined）
  get dishesSubtotal(): number {
    const amount = Number(this.diaForm.amount) || 0
    const pack = Number(this.diaForm.packAmount) || 0
    return amount - this.DELIVERY_FEE - pack
  }
  ...
  init(activeIndex: number = 0, isSearch?: boolean) {
    ...
    const params: any = {
      ...
      status: activeIndex || undefined // 0 代表全部，不传给后端
    }
  ...
  // 查看详情
  async goDetail(id: string | number, status: number, row?: OrderRow) {
    this.dialogVisible = true
    this.dialogOrderStatus = status
    this.orderId = id
    try {
      const { data }: any = await queryOrderDetailById({ orderId: id })
      this.diaForm = (data && data.data) || this.diaForm
   } catch (err) {
  const e = err as Error
  this.$message.error('获取详情失败：' + (e.message || '未知错误'))
}

    this.row = row || { id: this.$route.query.orderId as any, status, amount: 0 }
  ...
  confirmCancel() {
    ...
    const key = this.cancelDialogTitle === '取消' ? 'cancelReason' : 'rejectionReason'
    payload[key] = this.cancelReason === '自定义原因' ? this.remark : this.cancelReason

    ;(this.cancelDialogTitle === '取消' ? orderCancel : orderReject)(payload)
      .then((res: any) => {
        if (res.data.code === 1) {
          this.$message.success('操作成功')
  ...
  // 派送，完成
  cancelOrDeliveryOrComplete(status: number, id: string | number) {
